...
func (dataService *DataServices) GenerateAndSendOTP(ctx context.Context, shopId string, phoneNo string, resendTime int32, validTime time.Duration) error {
	otp := GenerateOTP()
	...
	err := dataService.SetDataToCash(ctx, shopId, data.Marshal(), validTime)
	if err != nil {
