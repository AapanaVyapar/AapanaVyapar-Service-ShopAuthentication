...
func (dataService *DataServices) LoadUserContactDataInCash(ctx context.Context) error {
	...
	for rows.Next() {
		...
		err = dataService.SetContactListDataToCash(ctx, detail.PhoneNo, detail.ShopId)
		if err != nil {
			return err
		}
...
func (dataService *DataServices) CreateTemporaryUserInCash(ctx context.Context, data *structs.UserData) error {
	...
	err := dataService.SetDataToCash(ctx, data.ShopId+"_Temp", data.Marshal(), TemporaryUserDetailsCashTiming)
	if err != nil {
		return err
	}
